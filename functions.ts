class Skill{
    ...
    constructor(skillName:string, logo:string, altText:string, carouselPosition:number){
...
function constructSkills(){
    let carousel = <HTMLElement> document.getElementById("carousel")
    carousel.innerHTML = ""
    for (let i in skills){
...
function updateSkills(){
    for (let i in skills){
        let currentSkill= <HTMLImageElement> document.getElementById(`${skills[i].skillName}`)
        if (skills[i].carouselPosition == (Math.ceil(skills.length/2) - 2)){
