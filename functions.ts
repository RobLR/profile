...
function constructSkills() {
	...
	for (let i in skills) {
		...
		if (skills[i].carouselPosition == Math.ceil(skills.length / 2) - 2) {
			...
			skills[i].carouselPosition ==
			Math.ceil(skills.length / 2) - 1
		) {
...
function updateSkills() {
	for (let i in skills) {
		let currentSkill = <HTMLImageElement>(
			document.getElementById(`${skills[i].skillName}`)
		);
		if (skills[i].carouselPosition == Math.ceil(skills.length / 2) - 2) {
